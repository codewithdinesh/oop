...
// Roll No : SA73
// Batch : C
// Assignment no. 6 : Vector Container//
...
public:
    ...
    int cost;
    int code;
    // returntype operator op(parameter) //code 1 2 3 4 5
    bool operator==(const Item &i1) // Boolean operators allow you to create more complex  conditional statements
    {
        if (code == i1.code) // operator will return 1 if the comparison is true, or 0 if the comparison is false  return 1;
            return 0;
    }
    bool operator<(const Item &i1)
    {
        if (cost < i1.cost) // operator will return 1 if the comparison is true, or 0 if the comparison is false  return 1;
            return 0;
    }
};
...
void search();
void dlt();
bool compare(const Item &i1, const Item &i2) // cost 100 50 300 40
{

    return i1.cost < i2.cost; // 100<50
}
int main()
{
    int ch;
